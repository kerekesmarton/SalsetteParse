...
- (BOOL)containsView:(UIView *)view {
    ...
    NSMutableSet *nextSubviews = [NSMutableSet set];
    while (subviews.count > 0) {
        [nextSubviews removeAllObjects];
        for (UIView *currentView in subviews) {
            NSSet *currentSubviews = [NSSet setWithArray:currentView.subviews];
            [nextSubviews unionSet:currentSubviews];
            if (currentView == view) {
                return YES;
            }
        }
        subviews = [NSMutableSet setWithSet:nextSubviews];
    }
    
    return NO;
}

- (void)conditionalySetAccessibilityIdentifier:(NSString *)identifier {
// it is empty so no Identifiers will overide a localizable string
}
